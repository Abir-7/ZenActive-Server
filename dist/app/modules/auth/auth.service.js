...
const loginUser = (userData) => __awaiter(void 0, void 0, void 0, function* () {
    const isUserExist = yield user_model_1.User.findOne({ email: userData.email }).select("+password");
...
const updatePassword = (userId, passData) => __awaiter(void 0, void 0, void 0, function* () {
    const isUserExist = yield user_model_1.User.findOne({ _id: userId }).select("+password");
    if (!isUserExist) {
        throw new AppError_1.default(http_status_1.default.NOT_FOUND, "User not found.");
    }
    if (isUserExist === null || isUserExist === void 0 ? void 0 : isUserExist.isBlocked) {
    ...
    if (!(yield user_model_1.User.passwordMatch(isUserExist.password, passData.old_password))) {
        throw new AppError_1.default(http_status_1.default.BAD_REQUEST, "Password not matched.");
    }
    if (passData.new_password !== passData.confirm_password) {
        throw new AppError_1.default(http_status_1.default.BAD_REQUEST, "Password not matched.");
    }
    const hashPassword = yield bcryptjs_1.default.hash(passData.new_password, Number(config_1.config.security.bcryptSaltRounds));
    const udpatePass = yield user_model_1.User.findOneAndUpdate({ _id: userId }, {
        password: hashPassword,
    }, { new: true });
    if (!udpatePass) {
        throw new AppError_1.default(http_status_1.default.BAD_REQUEST, "Failed to update password.");
    }
